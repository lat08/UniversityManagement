...
export default function WeeklySchedulePage() {
  ...
  const transformedSchedule = useMemo(() => {
    return scheduleData.map((item) => {
      // Map dayOfWeek tiếng Việt to number (2-8)
      const dayMap: Record<string, number> = {
        ...
        'thứ sáu': 6,
        'thứ bảy': 7,
        // Additional formats that might come from API (with trailing spaces)
        'thứ 2 ': 2,
        'thứ 3 ': 3,
        'thứ 4 ': 4,
        'thứ 5 ': 5,
        'thứ 6 ': 6,
        'thứ 7 ': 7,
        // Format with capital T (from API)
        'Thứ 2': 2,
        'Thứ 3': 3,
        'Thứ 4': 4,
        'Thứ 5': 5,
      ...
      // Try mapping first, then fallback to regex extraction
      let mappedDayOfWeek = dayMap[item.dayOfWeek.toLowerCase()];
      
      // If mapping failed, try to extract number from string
      if (mappedDayOfWeek === undefined) {
        const match = item.dayOfWeek.match(/(\d+)/);
        if (match) {
          const dayNum = parseInt(match[1]);
          if (dayNum >= 2 && dayNum <= 7) {
            mappedDayOfWeek = dayNum;
          } else {
            mappedDayOfWeek = 2; // default fallback
          }
        } else {
          mappedDayOfWeek = 2; // default fallback
        }
      ...
      return {
        ...
        periodsCount: item.endPeriod - item.startPeriod + 1,
    ...
    <div className="mx-auto max-w-[1600px] relative">
            ...
            <div className="mb-6 flex gap-4 items-stretch w-full mt-2">
              ...
              <div className="relative flex-1 dropdown-container">
                ...
                >
                  <span className="text-sm text-gray-900">
                    {viewType === 'week' ? 'Thời khóa biểu theo tuần' : 'Thời khóa biểu theo môn học'}
                  </span>
                  <ChevronDown className="w-4 h-4 ml-2 text-gray-700" />
                </button>
                {isViewTypeOpen && (
              ...
              <div className="mt-6 bg-red-50 border border-red-200 rounded-lg p-4">
                ...
                <div className="space-y-3">
                  {scheduleConflicts.map((conflict, index) => {
                    const dayName = DAYS_OF_WEEK.find(d => d.value === conflict.dayOfWeek)?.label || "";
