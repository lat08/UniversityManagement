...
export default function ExamScheduleManagementPage() {
  ...
  const [resizableColumns, setResizableColumns] = useState<ResizableColumn[]>([
    ...
    { key: 'subjectCode', label: 'Mã', width: 100, minWidth: 80, align: 'left', visible: true, required: true },
    { key: 'subjectName', label: 'Tên môn thi', width: 200, minWidth: 150, align: 'left', visible: true, required: true },
    { key: 'className', label: 'Lớp', width: 120, minWidth: 100, align: 'left', visible: true },
    { key: 'examDate', label: 'Ngày thi', width: 120, minWidth: 100, align: 'center', visible: true },
    { key: 'examTime', label: 'Giờ thi', width: 100, minWidth: 80, align: 'center', visible: true },
    { key: 'roomCode', label: 'Phòng thi', width: 120, minWidth: 100, align: 'left', visible: true },
    { key: 'examFormat', label: 'Hình thức', width: 150, minWidth: 120, align: 'left', visible: true },
    { key: 'status', label: 'Trạng thái', width: 140, minWidth: 120, align: 'center', visible: true },
    { key: 'actions', label: 'HĐ', width: 100, minWidth: 80, align: 'center', visible: true, required: true },
  ]);
  ...
  const handleEditClick = useCallback((examSchedule: ExamSchedule) => {
    if (examSchedule.status !== 'ready') {
      toast.error('Chỉ có thể sửa lịch thi ở trạng thái "Sẵn sàng"');
      return;
    }
    examSchedulesApi.getById(examSchedule.id).then((detail) => {
      ...
      toast.error('Không thể tải thông tin lịch thi');
    });
  }, []);
