feature_name: Instructor Profile
user_role: Admin
screens_and_ui_flow:
- Primary route: `app/[locale]/(page)/admin/instructor-profile/page.tsx` orchestrates data fetch, filters, and action triggers.
- Interactive components: `AddInstructorModal.tsx`, `EditInstructorModal.tsx`, `ExportInstructorModal.tsx`, `ImportInstructorExcelModal.tsx`, `InstructorActionsMenu.tsx`, `InstructorDetailModal.tsx`.
- Nested routes: `[id]` (mirror detail or sub-workflows).
- Modal flows: `AddInstructorModal.tsx`, `EditInstructorModal.tsx`, `ExportInstructorModal.tsx`, `ImportInstructorExcelModal.tsx`, `InstructorDetailModal.tsx`. Trigger from toolbar/row actions, submit via schema-backed forms, refetch queries on success, and close modals optimistically.
...
- lib/api: `instructorsApi.ts`.
- lib/constants: `filters.ts`, `statCards.ts`.
- lib/hooks: `useInstructors.ts`.
- lib/types: `types.ts`.
- lib/utils: `display.ts`, `string.ts`.
- Shared UI primitives live in `app/components/ui`; stay consistent with Button, Dropdown, SearchInput, ResizableTable, etc.
state_management:
- Data layer uses React Query via custom hooks (`useInstructors.ts`). Keep query keys in `lib/api/queryKeys` and memoize filters.
...
form_validations:
- Forms live inside `AddInstructorModal.tsx`, `EditInstructorModal.tsx`, `ExportInstructorModal.tsx`, `ImportInstructorExcelModal.tsx`, `InstructorDetailModal.tsx`. Use `react-hook-form` + `yup`/`zod`, surface translations via `next-intl`.
- Pre-fill edit modals via `reset` and keep date/number parsing centralized.
api_integration:
- REST clients live in lib/api (`instructorsApi.ts`). They wrap axios instance (`@/lib/api/client`) and normalize camelCase/PascalCase payloads.
...
- Pull copy from `next-intl` namespaces (e.g., `admin.instructor-profile`) and keep translation keys synchronized across components and modals.
