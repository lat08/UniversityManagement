feature_name: Division Management
user_role: Admin
screens_and_ui_flow:
- Primary route: `app/[locale]/(page)/admin/division-management/page.tsx` orchestrates data fetch, filters, and action triggers.
- Interactive components: `AddDivisionModal.tsx`, `BulkDeleteDivisionModal.tsx`, `BulkEditDivisionModal.tsx`, `ConfirmDeleteDivisionModal.tsx`, `DivisionActionsMenu.tsx`, `DivisionStatCard.tsx`, `EditDivisionModal.tsx`.
- Modal flows: `AddDivisionModal.tsx`, `BulkDeleteDivisionModal.tsx`, `BulkEditDivisionModal.tsx`, `ConfirmDeleteDivisionModal.tsx`, `EditDivisionModal.tsx`. Trigger from toolbar/row actions, submit via schema-backed forms, refetch queries on success, and close modals optimistically.
...
- lib/api: `divisionsApi.ts`.
- lib/hooks: `useDivisions.ts`.
- lib/types: `types.ts`.
- Shared UI primitives live in `app/components/ui`; stay consistent with Button, Dropdown, SearchInput, ResizableTable, etc.
state_management:
- Data layer uses React Query via custom hooks (`useDivisions.ts`). Keep query keys in `lib/api/queryKeys` and memoize filters.
...
form_validations:
- Forms live inside `AddDivisionModal.tsx`, `BulkDeleteDivisionModal.tsx`, `BulkEditDivisionModal.tsx`, `ConfirmDeleteDivisionModal.tsx`, `EditDivisionModal.tsx`. Use `react-hook-form` + `yup`/`zod`, surface translations via `next-intl`.
- Pre-fill edit modals via `reset` and keep date/number parsing centralized.
api_integration:
- REST clients live in lib/api (`divisionsApi.ts`). They wrap axios instance (`@/lib/api/client`) and normalize camelCase/PascalCase payloads.
...
- Pull copy from `next-intl` namespaces (e.g., `admin.division-management`) and keep translation keys synchronized across components and modals.
