feature_name: Building Management
user_role: Admin
screens_and_ui_flow:
- Primary route: `app/[locale]/(page)/admin/building-management/page.tsx` orchestrates data fetch, filters, and action triggers.
- Interactive components: `AddBuildingModal.tsx`, `BuildingActionsMenu.tsx`, `BuildingStatCard.tsx`, `BulkDeleteBuildingModal.tsx`, `BulkEditBuildingModal.tsx`, `ConfirmDeleteBuildingModal.tsx`, `EditBuildingModal.tsx`.
- Modal flows: `AddBuildingModal.tsx`, `BulkDeleteBuildingModal.tsx`, `BulkEditBuildingModal.tsx`, `ConfirmDeleteBuildingModal.tsx`, `EditBuildingModal.tsx`. Trigger from toolbar/row actions, submit via schema-backed forms, refetch queries on success, and close modals optimistically.
...
- lib/api: `buildingsApi.ts`.
- lib/hooks: `useBuildings.ts`.
- lib/types: `types.ts`.
- Shared UI primitives live in `app/components/ui`; stay consistent with Button, Dropdown, SearchInput, ResizableTable, etc.
state_management:
- Data layer uses React Query via custom hooks (`useBuildings.ts`). Keep query keys in `lib/api/queryKeys` and memoize filters.
...
form_validations:
- Forms live inside `AddBuildingModal.tsx`, `BulkDeleteBuildingModal.tsx`, `BulkEditBuildingModal.tsx`, `ConfirmDeleteBuildingModal.tsx`, `EditBuildingModal.tsx`. Use `react-hook-form` + `yup`/`zod`, surface translations via `next-intl`.
- Pre-fill edit modals via `reset` and keep date/number parsing centralized.
api_integration:
- REST clients live in lib/api (`buildingsApi.ts`). They wrap axios instance (`@/lib/api/client`) and normalize camelCase/PascalCase payloads.
...
- Pull copy from `next-intl` namespaces (e.g., `admin.building-management`) and keep translation keys synchronized across components and modals.
