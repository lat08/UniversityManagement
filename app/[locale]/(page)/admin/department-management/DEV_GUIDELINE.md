...
user_role: Admin
screens_and_ui_flow:
- Primary route: `app/[locale]/(page)/admin/department-management/page.tsx` orchestrates data fetch, filters, and action triggers.
- Interactive components: `AddDepartmentModal.tsx`, `BulkDeleteDepartmentModal.tsx`, `BulkEditDepartmentModal.tsx`, `ConfirmDeleteDepartmentModal.tsx`, `DepartmentActionsMenu.tsx`, `DepartmentCurriculaModal.tsx`, `DepartmentStatCard.tsx`, `EditDepartmentModal.tsx`.
- Modal flows: `AddDepartmentModal.tsx`, `BulkDeleteDepartmentModal.tsx`, `BulkEditDepartmentModal.tsx`, `ConfirmDeleteDepartmentModal.tsx`, `DepartmentCurriculaModal.tsx`, `EditDepartmentModal.tsx`. Trigger from toolbar/row actions, submit via schema-backed forms, refetch queries on success, and close modals optimistically.
...
state_management:
- Data layer uses React Query via custom hooks (`useDepartments.ts`). Keep query keys in `lib/api/queryKeys` and memoize filters.
...
form_validations:
- Forms live inside `AddDepartmentModal.tsx`, `BulkDeleteDepartmentModal.tsx`, `BulkEditDepartmentModal.tsx`, `ConfirmDeleteDepartmentModal.tsx`, `DepartmentCurriculaModal.tsx`, `EditDepartmentModal.tsx`. Use `react-hook-form` + `yup`/`zod`, surface translations via `next-intl`.
- Pre-fill edit modals via `reset` and keep date/number parsing centralized.
api_integration:
- REST clients live in lib/api (`departmentsApi.ts`). They wrap axios instance (`@/lib/api/client`) and normalize camelCase/PascalCase payloads.
...
- Pull copy from `next-intl` namespaces (e.g., `admin.department-management`) and keep translation keys synchronized across components and modals.
