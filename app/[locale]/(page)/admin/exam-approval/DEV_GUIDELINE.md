feature_name: Exam Approval
user_role: Admin
screens_and_ui_flow:
- Primary route: `app/[locale]/(page)/admin/exam-approval/page.tsx` orchestrates data fetch, filters, and action triggers.
- Interactive components: `BulkApproveModal.tsx`, `BulkRejectModal.tsx`, `ExamActionsMenu.tsx`, `ExamDetailModal.tsx`, `ExamFilters.tsx`, `RejectExamModal.tsx`.
- Modal flows: `BulkApproveModal.tsx`, `BulkRejectModal.tsx`, `ExamDetailModal.tsx`, `RejectExamModal.tsx`. Trigger from toolbar/row actions, submit via schema-backed forms, refetch queries on success, and close modals optimistically.
...
form_validations:
- Forms live inside `BulkApproveModal.tsx`, `BulkRejectModal.tsx`, `ExamDetailModal.tsx`, `RejectExamModal.tsx`. Use `react-hook-form` + `yup`/`zod`, surface translations via `next-intl`.
...
- Pull copy from `next-intl` namespaces (e.g., `admin.exam-approval`) and keep translation keys synchronized across components and modals.
