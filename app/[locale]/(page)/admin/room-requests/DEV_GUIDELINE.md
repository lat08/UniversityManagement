feature_name: Room Requests
user_role: Admin
screens_and_ui_flow:
- Primary route: `app/[locale]/(page)/admin/room-requests/page.tsx` orchestrates data fetch, filters, and action triggers.
- Interactive components: `ApproveRoomRequestModal.tsx`, `BulkApproveRoomRequestsModal.tsx`, `BulkRejectRoomRequestsModal.tsx`, `CancelRoomRequestModal.tsx`, `MajorStatCard.tsx`, `RejectRoomRequestModal.tsx`, `RoomRequestActionsMenu.tsx`, `RoomRequestDetailModal.tsx`.
- Modal flows: `ApproveRoomRequestModal.tsx`, `BulkApproveRoomRequestsModal.tsx`, `BulkRejectRoomRequestsModal.tsx`, `CancelRoomRequestModal.tsx`, `RejectRoomRequestModal.tsx`, `RoomRequestDetailModal.tsx`. Trigger from toolbar/row actions, submit via schema-backed forms, refetch queries on success, and close modals optimistically.
...
form_validations:
- Forms live inside `ApproveRoomRequestModal.tsx`, `BulkApproveRoomRequestsModal.tsx`, `BulkRejectRoomRequestsModal.tsx`, `CancelRoomRequestModal.tsx`, `RejectRoomRequestModal.tsx`, `RoomRequestDetailModal.tsx`. Use `react-hook-form` + `yup`/`zod`, surface translations via `next-intl`.
...
- Pull copy from `next-intl` namespaces (e.g., `admin.room-requests`) and keep translation keys synchronized across components and modals.
