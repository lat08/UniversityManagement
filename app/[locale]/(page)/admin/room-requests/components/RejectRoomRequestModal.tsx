...
interface RejectRoomRequestModalProps {
...
export const RejectRoomRequestModal = ({
  isOpen,
  request,
  onClose,
  onConfirm,
}: RejectRoomRequestModalProps) => {
  const t = useTranslations('admin.roomRequests.rejectModal');
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[500px] [&>button[data-radix-dialog-close]:not(.custom-close)]:hidden">
        ...
        <div className="space-y-4">
          <p className="text-sm text-gray-700">{t('instruction')}</p>
