...
user_role: Admin
screens_and_ui_flow:
- Primary route: `app/[locale]/(page)/admin/tuition/page.tsx` orchestrates data fetch, filters, and action triggers.
- Interactive components: `TuitionStatCard.tsx`, `bulk-action-bar.tsx`, `bulk-delete-tuition-modal.tsx`, `create-reminder-modal.tsx`, `edit-tuition-modal.tsx`, `tuition-debt-actions-menu.tsx`, `tuition-debt-detail-modal.tsx`.
- Modal flows: `bulk-delete-tuition-modal.tsx`, `create-reminder-modal.tsx`, `edit-tuition-modal.tsx`, `tuition-debt-detail-modal.tsx`. Trigger from toolbar/row actions, submit via schema-backed forms, refetch queries on success, and close modals optimistically.
...
form_validations:
- Forms live inside `bulk-delete-tuition-modal.tsx`, `create-reminder-modal.tsx`, `edit-tuition-modal.tsx`, `tuition-debt-detail-modal.tsx`. Use `react-hook-form` + `yup`/`zod`, surface translations via `next-intl`.
...
- Pull copy from `next-intl` namespaces (e.g., `admin.tuition`) and keep translation keys synchronized across components and modals.
