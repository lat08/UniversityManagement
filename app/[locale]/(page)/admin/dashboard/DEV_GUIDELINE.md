...
user_role: Admin
screens_and_ui_flow:
- Primary route: `app/[locale]/(page)/admin/dashboard/page.tsx` orchestrates data fetch, filters, and action triggers.
- Interactive components: `content\AdminDashboardContent.tsx`, `pendingTaskCard.tsx`, `quickActionButton.tsx`, `recentUpdateCard.tsx`, `statsCard.tsx`.
- Entry screens must set page titles via `usePageTitle` and wrap async sections with skeleton fallbacks.
component_structure:
- UI layer split across: 5 components under `components`. Prioritize composition from table/list containers down to action menus.
...
state_management:
- Data layer uses React Query via custom hooks (`useAdminDashboard.ts`). Keep query keys in `lib/api/queryKeys` and memoize filters.
...
- Pull copy from `next-intl` namespaces (e.g., `admin.dashboard`) and keep translation keys synchronized across components and modals.
