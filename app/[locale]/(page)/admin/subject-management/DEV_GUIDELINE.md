...
user_role: Admin
screens_and_ui_flow:
- Primary route: `app/[locale]/(page)/admin/subject-management/page.tsx` orchestrates data fetch, filters, and action triggers.
- Interactive components: `AddSubjectModal.tsx`, `BulkDeleteModal.tsx`, `BulkEditModal.tsx`, `DeleteConfirmModal.tsx`, `EditSubjectModal.tsx`, `SubjectActionsMenu.tsx`.
- Modal flows: `AddSubjectModal.tsx`, `BulkDeleteModal.tsx`, `BulkEditModal.tsx`, `DeleteConfirmModal.tsx`, `EditSubjectModal.tsx`. Trigger from toolbar/row actions, submit via schema-backed forms, refetch queries on success, and close modals optimistically.
...
state_management:
- Data layer uses React Query via custom hooks (`useSubjects.ts`). Keep query keys in `lib/api/queryKeys` and memoize filters.
...
form_validations:
- Forms live inside `AddSubjectModal.tsx`, `BulkDeleteModal.tsx`, `BulkEditModal.tsx`, `DeleteConfirmModal.tsx`, `EditSubjectModal.tsx`. Use `react-hook-form` + `yup`/`zod`, surface translations via `next-intl`.
- Pre-fill edit modals via `reset` and keep date/number parsing centralized.
api_integration:
- REST clients live in lib/api (`subjectsApi.ts`). They wrap axios instance (`@/lib/api/client`) and normalize camelCase/PascalCase payloads.
...
- Pull copy from `next-intl` namespaces (e.g., `admin.subject-management`) and keep translation keys synchronized across components and modals.
