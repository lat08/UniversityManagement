feature_name: Class Management
user_role: Admin
screens_and_ui_flow:
- Primary route: `app/[locale]/(page)/admin/class-management/page.tsx` orchestrates data fetch, filters, and action triggers.
- Interactive components: `AddClassModal.tsx`, `ConfirmDeleteClassModal.tsx`, `EditClassModal.tsx`, `ViewClassDetailModal.tsx`.
- Modal flows: `AddClassModal.tsx`, `ConfirmDeleteClassModal.tsx`, `EditClassModal.tsx`, `ViewClassDetailModal.tsx`. Trigger from toolbar/row actions, submit via schema-backed forms, refetch queries on success, and close modals optimistically.
- Entry screens must set page titles via `usePageTitle` and wrap async sections with skeleton fallbacks.
component_structure:
- UI layer split across: 4 components under `components`. Prioritize composition from table/list containers down to action menus.
- lib/api: `classesApi.ts`.
- lib/types: `types.ts`.
...
form_validations:
- Forms live inside `AddClassModal.tsx`, `ConfirmDeleteClassModal.tsx`, `EditClassModal.tsx`, `ViewClassDetailModal.tsx`. Use `react-hook-form` + `yup`/`zod`, surface translations via `next-intl`.
- Pre-fill edit modals via `reset` and keep date/number parsing centralized.
api_integration:
- REST clients live in lib/api (`classesApi.ts`). They wrap axios instance (`@/lib/api/client`) and normalize camelCase/PascalCase payloads.
...
- Pull copy from `next-intl` namespaces (e.g., `admin.class-management`) and keep translation keys synchronized across components and modals.
