feature_name: Theme Configuration
user_role: Admin
screens_and_ui_flow:
- Primary route: `app/[locale]/(page)/admin/theme-configuration/page.tsx` orchestrates data fetch, filters, and action triggers.
- Interactive components: `ColorPicker.tsx`, `PageThemeEditor.tsx`, `ThemeCard.tsx`.
...
state_management:
- Data layer uses React Query via custom hooks (`useThemeSignalR.ts`). Keep query keys in `lib/api/queryKeys` and memoize filters.
...
api_integration:
- REST clients live in lib/api (`theme.ts`). They wrap axios instance (`@/lib/api/client`) and normalize camelCase/PascalCase payloads.
...
- Pull copy from `next-intl` namespaces (e.g., `admin.theme-configuration`) and keep translation keys synchronized across components and modals.
