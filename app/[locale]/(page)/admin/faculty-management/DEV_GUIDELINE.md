feature_name: Faculty Management
user_role: Admin
screens_and_ui_flow:
- Primary route: `app/[locale]/(page)/admin/faculty-management/page.tsx` orchestrates data fetch, filters, and action triggers.
- Interactive components: `AddFacultyModal.tsx`, `BulkDeleteFacultyModal.tsx`, `BulkEditFacultyModal.tsx`, `ConfirmDeleteFacultyModal.tsx`, `EditFacultyModal.tsx`, `FacultyActionsMenu.tsx`, `FacultyStatCard.tsx`.
- Modal flows: `AddFacultyModal.tsx`, `BulkDeleteFacultyModal.tsx`, `BulkEditFacultyModal.tsx`, `ConfirmDeleteFacultyModal.tsx`, `EditFacultyModal.tsx`. Trigger from toolbar/row actions, submit via schema-backed forms, refetch queries on success, and close modals optimistically.
...
- lib/api: `facultiesApi.ts`.
- lib/hooks: `useFaculties.ts`.
- lib/types: `types.ts`.
- Shared UI primitives live in `app/components/ui`; stay consistent with Button, Dropdown, SearchInput, ResizableTable, etc.
state_management:
- Data layer uses React Query via custom hooks (`useFaculties.ts`). Keep query keys in `lib/api/queryKeys` and memoize filters.
...
form_validations:
- Forms live inside `AddFacultyModal.tsx`, `BulkDeleteFacultyModal.tsx`, `BulkEditFacultyModal.tsx`, `ConfirmDeleteFacultyModal.tsx`, `EditFacultyModal.tsx`. Use `react-hook-form` + `yup`/`zod`, surface translations via `next-intl`.
- Pre-fill edit modals via `reset` and keep date/number parsing centralized.
api_integration:
- REST clients live in lib/api (`facultiesApi.ts`). They wrap axios instance (`@/lib/api/client`) and normalize camelCase/PascalCase payloads.
...
- Pull copy from `next-intl` namespaces (e.g., `admin.faculty-management`) and keep translation keys synchronized across components and modals.
