...
export default function ScheduleChangeManagementPage() {
  const t = useTranslations('admin.scheduleChangeManagement');
  ...
  const [resizableColumns, setResizableColumns] = useState<ResizableColumn[]>([
    ...
    { key: 'requestCode', label: t('list.columns.requestCode'), width: 120, minWidth: 100, align: 'left', visible: true, required: true },
    { key: 'instructor', label: t('list.columns.instructor'), width: 200, minWidth: 150, align: 'left', visible: true },
    { key: 'subject', label: t('list.columns.subject'), width: 200, minWidth: 150, align: 'left', visible: true },
    { key: 'class', label: t('list.columns.class'), width: 120, minWidth: 100, align: 'left', visible: true },
    { key: 'currentSchedule', label: t('list.columns.currentSchedule'), width: 250, minWidth: 200, align: 'left', visible: true },
    { key: 'proposedSchedule', label: t('list.columns.proposedSchedule'), width: 250, minWidth: 200, align: 'left', visible: true },
    { key: 'status', label: t('list.columns.status'), width: 140, minWidth: 120, align: 'center', visible: true },
    { key: 'actions', label: t('list.columns.actions'), width: 100, minWidth: 80, align: 'center', visible: true, required: true },
  ]);
  ...
  const handleExport = async () => {
    try {
      ...
      const errorMessage = apiError?.response?.data?.message || apiError?.message || t('toast.exportError');
  ...
  const renderRequestRow = useCallback((request: LeaveRequest, visibleColumns: ResizableColumn[], cellStyle: { paddingX: string; paddingY: string }) => {
    const statusDisplay = getStatusDisplay(request.status, (key: string) => t(key));
    ...
    const dayLabel = request.dayOfWeekText || t('list.dayOfWeek', { day: request.dayOfWeek });
                ...
                <td key="currentSchedule" style={{ ...cellPaddingStyle, overflow: 'hidden' }}>
                  {cancelledDate ? (
                    <div className="flex flex-col">
                      <span className="text-gray-900">{format(cancelledDate, 'dd/MM/yyyy', { locale: dateLocale })}</span>
                      <span className="text-gray-500 text-sm">
                        {getPeriodLabel(request.startPeriod, request.endPeriod, translateSchedule)}
                        {request.currentRoomCode && ` (${request.currentRoomCode})`}
                      </span>
                      <span className="text-gray-500 text-xs">{t('list.week', { week: request.cancelledWeek })}</span>
                    </div>
                  ) : (
                    <div className="flex flex-col">
            ...
            case 'proposedSchedule':
              return (
                <td key="proposedSchedule" style={{ ...cellPaddingStyle, overflow: 'hidden' }}>
                  {makeUpDate ? (
                    <div className="flex flex-col">
                      <span className="text-gray-900">{format(makeUpDate, 'dd/MM/yyyy', { locale: dateLocale })}</span>
                      <span className="text-gray-500 text-sm">
                        {getPeriodLabel(request.startPeriod, request.endPeriod, translateSchedule)}
                        {request.makeUpRoomCode && ` (${request.makeUpRoomCode})`}
                      </span>
                      {request.makeupWeek && (
                        <span className="text-gray-500 text-xs">{t('list.makeupWeek', { week: request.makeupWeek })}</span>
                      )}
                    </div>
                  ) : request.makeupWeek ? (
                    <div className="flex flex-col">
                      <span className="text-gray-900">{t('list.makeupWeek', { week: request.makeupWeek })}</span>
                      <span className="text-gray-500 text-sm">
  ...
  return (
    <div className="space-y-4 lg:space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">{t('pageTitle')}</h1>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div className="p-4 lg:p-6 border-b border-gray-200 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">
            ...
            {/* Status Dropdown */}
            <Dropdown
              options={STATUS_OPTIONS.map(opt => ({ ...opt, label: t(`status.${opt.value || 'all'}`) }))}
              value={selectedStatus || ''}
              placeholder={t('list.statusAll')}
              onChange={(value) => {
            ...
            <div className="flex items-center justify-between p-3 bg-[#E8F4FF] border border-[#0053AD]/20 rounded-lg">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-[#0053AD]">
                  {t('list.bulkActions.selected', { count: selectedRequestIds.size })}
                </span>
              </div>
              <div className="flex gap-2">
                <Button
