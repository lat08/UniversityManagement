...
export const getStatusDisplay = (status: string, t?: (key: string) => string) => {
  ...
  const statusMap: Record<string, { label: string; color: string }> = {
    pending: { 
      label: t ? t('status.pending') : 'Chờ duyệt', 
      color: 'bg-orange-100 text-orange-700' 
    },
    approved: { 
      label: t ? t('status.approved') : 'Đã duyệt', 
      color: 'bg-green-100 text-green-700' 
    },
    rejected: { 
      label: t ? t('status.rejected') : 'Từ chối', 
      color: 'bg-red-100 text-red-700' 
    },
  };
  return statusMap[statusLower] || { 
    label: status || (t ? t('status.unknown') : 'Không xác định'), 
...
export const getPeriodLabel = (startPeriod: number, endPeriod: number, t?: (key: string, params?: Record<string, unknown>) => string): string => {
  if (startPeriod === endPeriod) {
    return t ? t('grid.period', { period: startPeriod }) : `Tiết ${startPeriod}`;
  }
  return t ? t('grid.periodRange', { start: startPeriod, end: endPeriod }) : `Tiết ${startPeriod}-${endPeriod}`;
};
