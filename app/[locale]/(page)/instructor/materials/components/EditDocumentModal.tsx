...
export function EditDocumentModal({
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.subjectClass) {
      newErrors.subjectClass = tUpload('subjectClassRequired');
    }
    if (!formData.documentType) {
      newErrors.documentType = tUpload('documentTypeRequired');
    }
    if (!formData.documentName.trim()) {
      newErrors.documentName = tUpload('documentNameRequired');
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            {isLoadingCourseClasses ? (
              ...
              <DropdownSearch
                ...
                placeholder={tUpload('selectSubjectClass')}
          ...
          <div className="space-y-2">
            ...
            <Dropdown
              ...
              placeholder={tUpload('selectDocumentType')}
              buttonClassName="border-gray-300"
              disabled={isUpdating}
            />
            {errors.documentType && (
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-gray-900 flex items-center gap-1">
              {tUpload('documentName')}
              <span className="text-red-500">*</span>
            </label>
            <Input
              type="text"
              placeholder={tUpload('documentNamePlaceholder')}
              ...
              <div className="flex flex-col items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-gray-700">
                    {tUpload('dragDrop')} <span className="text-blue-600 underline">{tUpload('clickToUpload')}</span>
                  </p>
                  {hasNewFile && (
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            <Button
              ...
              disabled={isUpdating}
            >
              {isUpdating ? t('updating') : t('submit')}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
