...
screens_and_ui_flow:
- Primary route: `app/[locale]/(page)/instructor/grades/page.tsx` orchestrates data fetch, filters, and action triggers.
- Interactive components: `ActionButtons.tsx`, `CourseClassSelector.tsx`, `GradeHistoryTable.tsx`, `GradeVersionModal.tsx`, `GradesTable.tsx`, `GradesTableSkeleton.tsx`, `StatsCards.tsx`, `StatsSkeleton.tsx`, `SubmitApprovalDialog.tsx`.
- Modal flows: `GradeVersionModal.tsx`. Trigger from toolbar/row actions, submit via schema-backed forms, refetch queries on success, and close modals optimistically.
...
form_validations:
- Forms live inside `GradeVersionModal.tsx`. Use `react-hook-form` + `yup`/`zod`, surface translations via `next-intl`.
...
- Pull copy from `next-intl` namespaces (e.g., `instructor.grades`) and keep translation keys synchronized across components and modals.
