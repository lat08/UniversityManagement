...
screens_and_ui_flow:
- Primary route: `app/[locale]/(page)/instructor/dashboard/page.tsx` orchestrates data fetch, filters, and action triggers.
- Interactive components: `DashboardStatCard.tsx`, `RemindersSection.tsx`, `WeeklyScheduleTimeline.tsx`, `dashboard-skeleton.tsx`.
...
- lib/hooks: `useDashboard.ts`, `useDashboardRealtime.ts`.
- lib/types: `types.ts`.
- Shared UI primitives live in `app/components/ui`; stay consistent with Button, Dropdown, SearchInput, ResizableTable, etc.
state_management:
- Data layer uses React Query via custom hooks (`useDashboard.ts`, `useDashboardRealtime.ts`). Keep query keys in `lib/api/queryKeys` and memoize filters.
...
api_integration:
- REST clients live in lib/api (`dashboardApi.ts`). They wrap axios instance (`@/lib/api/client`) and normalize camelCase/PascalCase payloads.
...
- Pull copy from `next-intl` namespaces (e.g., `instructor.dashboard`) and keep translation keys synchronized across components and modals.
