...
const createValidateFile = (t: (key: string) => string) => (file: File | null): string | null => {
  if (!file) return null;
  
  if (file.size > MAX_FILE_SIZE) {
    return t('uploadModal.fileSizeError');
  }
  
  if (!ALLOWED_FILE_TYPES.includes(file.type)) {
    return t('uploadModal.fileTypeError');
  }
...
export function UploadExamModal({
  ...
  const validateFile = useMemo(() => createValidateFile(t), [t]);

  const examTypes = useMemo(() => [
    { id: "midterm", name: tExamTypes('midterm') },
    { id: "final", name: tExamTypes('final') },
    { id: "quiz", name: tExamTypes('quiz') },
    { id: "makeup", name: tExamTypes('makeup') },
  ], [tExamTypes]);
  ...
  const validateForm = (): boolean => {
    ...
    if (!isResubmitMode) {
      if (!formData.courseClassId) {
        newErrors.courseClassId = tModal('subjectRequired');
      }

      if (!formData.examType) {
        newErrors.examType = tModal('examTypeRequired');
      }
    }

    if (!formData.questionFile) {
      newErrors.questionFile = tModal('questionFileRequired');
    }

    if (!formData.answerFile) {
      newErrors.answerFile = tModal('answerFileRequired');
    }
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold text-gray-900">
            {isResubmitMode ? tModal('resubmitTitle') : tModal('title')}
          </DialogTitle>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
              ...
              <Dropdown
                ...
                placeholder={
                  courseClasses.length === 0 
                    ? tModal('loadingCourseClasses')
                    : tModal('selectCourseClass')
                }
            ...
            {courseClasses.length === 0 && (
              <p className="text-xs text-gray-500">{tModal('waitingCourseClasses')}</p>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            {isResubmitMode && existingExam ? (
              <div className="px-3 py-2 bg-gray-100 rounded-md text-gray-700">
                {tExamTypes(existingExam.examType as 'midterm' | 'final' | 'quiz' | 'makeup') || existingExam.examType}
              </div>
            ) : (
              <Dropdown
                options={examTypes.map(t => ({ value: t.id, label: t.name }))}
                value={formData.examType}
                placeholder={tModal('selectExamType')}
          ...
          <div className="space-y-2">
            ...
            {isResubmitMode && existingExam ? (
              ...
              <Input
                ...
                placeholder={tModal('durationPlaceholder')}
              />
            )}
          </div>
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            >
              {isSubmitting ? tModal('uploading') : tModal('upload')}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
