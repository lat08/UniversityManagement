...
const ExamCardComponent = ({
  ...
  const statusLabel = useMemo(() => {
    return tStatuses(exam.entryStatus as 'approved' | 'pending' | 'rejected') || ENTRY_STATUS_LABELS[exam.entryStatus] || exam.entryStatus;
  }, [exam.entryStatus, tStatuses]);
  
  const statusClassName = ENTRY_STATUS_COLORS[exam.entryStatus] || "bg-gray-100 text-gray-700";
  
  const examTypeLabel = useMemo(() => {
    return tExamTypes(exam.examType as 'midterm' | 'final' | 'quiz' | 'makeup') || EXAM_TYPE_LABELS[exam.examType] || exam.examType;
  }, [exam.examType, tExamTypes]);
      ...
      <div className="flex flex-col sm:flex-row items-start gap-3 sm:gap-4">
        ...
        <div className="flex-1 min-w-0 w-full">
          ...
          <div className="space-y-0.5 sm:space-y-1 text-xs sm:text-sm text-gray-600">
            ...
            <p>{tCard('duration')}: {exam.durationMinutes} {tCard('minutes')}</p>
            <p>{tCard('createdAt')}: {exam.createdAt ? formatDate(exam.createdAt) : 'N/A'}</p>
            {exam.reviewerName && (
              <p>{tCard('reviewer')}: {exam.reviewerName}</p>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap w-full sm:w-auto sm:flex-shrink-0">
            ...
            <>
              {exam.questionFilePath && (
                ...
                >
                  ...
                  <span className="truncate">{tCard('downloadQuestion')}</span>
                </Button>
              )}
              {exam.answerFilePath && (
                <Button
                ...
                >
                  <Download className="w-3.5 h-3.5 sm:w-4 sm:h-4 flex-shrink-0" />
                  <span className="truncate">{tCard('downloadAnswer')}</span>
                </Button>
              )}
            </>
          )}
          {exam.entryStatus === "rejected" && (
            ...
            >
              ...
              <span className="truncate">{tCard('resubmit')}</span>
            </Button>
          )}
          <Button
            variant="outline"
          ...
          >
            ...
            <span className="truncate">{tCard('view')}</span>
          </Button>
        </div>
      </div>
        ...
        <div className="mt-3">
          <div className="bg-red-50 border border-red-200 rounded-lg p-2.5 sm:p-3">
            <p className="text-xs sm:text-sm text-red-800">
              <span className="font-semibold">{tCard('rejectionReason')}</span> {exam.rejectionReason}
            </p>
          </div>
        </div>
      )}
