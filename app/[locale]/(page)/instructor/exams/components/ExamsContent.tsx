...
export const ExamsContent = () => {
  ...
  const [selectedExamForResubmit, setSelectedExamForResubmit] = useState<ExamEntry | null>(null);
  ...
  if (examEntries.length === 0 && loading) {
    return (
      <div key="exams-loading-skeleton" className="animate-in fade-in duration-100">
        <ExamsSkeleton />
      </div>
    );
  }
  ...
  return (
    <div key="exams-content" className="animate-in fade-in duration-200 space-y-4 sm:space-y-6">
      <ExamsHeader onUploadClick={handleUpload} />
      ...
      <div className="space-y-4">
        {examEntries.length === 0 ? (
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-8 text-center">
            <p className="text-gray-600">{t('empty.noExams')}</p>
          </div>
        ) : (
      ...
      {isUploadModalOpen && (
        <Suspense fallback={<div>{t('detailModal.loading')}</div>}>
          <UploadExamModal
