...
screens_and_ui_flow:
- Primary route: `app/[locale]/(page)/instructor/profile/page.tsx` orchestrates data fetch, filters, and action triggers.
- Interactive components: `ChangePasswordForm.tsx`, `ProfileAvatar.tsx`, `ProfileField.tsx`, `ProfileSkeleton.tsx`.
...
state_management:
- Data layer uses React Query via custom hooks (`useProfile.ts`). Keep query keys in `lib/api/queryKeys` and memoize filters.
...
form_validations:
- Forms live inside `ChangePasswordForm.tsx`. Use `react-hook-form` + `yup`/`zod`, surface translations via `next-intl`.
- Pre-fill edit modals via `reset` and keep date/number parsing centralized.
api_integration:
- REST clients live in lib/api (`profileApi.ts`). They wrap axios instance (`@/lib/api/client`) and normalize camelCase/PascalCase payloads.
...
- Pull copy from `next-intl` namespaces (e.g., `instructor.profile`) and keep translation keys synchronized across components and modals.
