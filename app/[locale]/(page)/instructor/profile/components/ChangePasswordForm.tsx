...
export const ChangePasswordForm = memo<ChangePasswordFormProps>(({
  ...
  const t = useTranslations('instructor.profile.changePassword')
  ...
  const validatePassword = (password: string): string[] => {
    const rules: Array<{ condition: boolean; message: string }> = [
      { condition: password.length < 8, message: t('rules.minLength') },
      { condition: !/[a-z]/.test(password), message: t('rules.lowercase') },
      { condition: !/[A-Z]/.test(password), message: t('rules.uppercase') },
      { condition: !/\d/.test(password), message: t('rules.number') },
      { condition: !/[@$!%*?&]/.test(password), message: t('rules.symbol') },
    ]

    return rules.filter(rule => rule.condition).map(rule => rule.message)
  }
  ...
  const validateForm = () => {
    ...
    if (!formData.currentPassword.trim()) {
      newErrors.currentPassword = t('validation.currentRequired')
    }

    if (!formData.newPassword.trim()) {
      newErrors.newPassword = t('validation.newRequired')
    } else {
    ...
    if (formData.currentPassword === formData.newPassword) {
      newErrors.newPassword = t('validation.sameAsCurrent')
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        <div className="mb-6">
          <h2 className="text-base font-bold mb-2">{t('title')}</h2>
        </div>
          ...
          <div className="space-y-2">
            ...
            {allPasswordErrors.length > 0 && formData.newPassword && (
              <div className="mt-2 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                <p className="text-xs font-medium text-yellow-900 mb-2">{t('passwordHintTitle')}:</p>
            ...
            {allPasswordErrors.length === 0 && formData.newPassword && (
              <div className="mt-2 p-3 bg-green-50 border border-green-200 rounded-lg">
                <p className="text-xs text-green-700">{t('passwordHintValid')}</p>
              </div>
            )}
          </div>
