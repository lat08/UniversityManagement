...
screens_and_ui_flow:
- Primary route: `app/[locale]/(page)/instructor/notification/page.tsx` orchestrates data fetch, filters, and action triggers.
- Interactive components: `content\notificationContent.tsx`.
- Entry screens must set page titles via `usePageTitle` and wrap async sections with skeleton fallbacks.
component_structure:
- UI layer split across: 1 components under `components`. Prioritize composition from table/list containers down to action menus.
...
- Pull copy from `next-intl` namespaces (e.g., `instructor.notification`) and keep translation keys synchronized across components and modals.
