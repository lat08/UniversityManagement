...
export function CourseCard({
  ...
  const t = useTranslations('student.course.card')
        ...
        <div className="flex-1 space-y-3">
          ...
          <div className="space-y-2 text-sm text-gray-600" style={{marginLeft: '10px'}}>
            <div className="flex items-center gap-2">
              ...
              <span>{t('instructor', { name: instructorName, code: instructorCode })}</span>
            </div>
            <div className="flex items-center gap-2">
              <MapPin className="h-4 w-4 flex-shrink-0" />
              <span>{t('location', { roomCode: schedule?.roomCode ?? '-', roomName: schedule?.roomName ?? '-' })}</span>
            </div>
            <div className="flex items-center gap-2">
              <Calendar className="h-4 w-4 flex-shrink-0" />
              <span>{t('dateRange', { startDate: formattedStartDate, endDate: formattedEndDate })}</span>
            </div>
            <div className="flex items-center gap-2">
              <Clock className="h-4 w-4 flex-shrink-0" />
              <span>{t('schedule', { day: schedule?.dayOfWeekName ?? '-', time: schedule?.timeRange ?? '-' })}</span>
            </div>
            
            <div className="flex items-center gap-2">
              <User className="h-4 w-4 flex-shrink-0" />
              <span>{t('status', { status: registrationStatus })}</span>
            </div>
