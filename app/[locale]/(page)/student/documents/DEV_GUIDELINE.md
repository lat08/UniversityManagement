...
user_role: Student
screens_and_ui_flow:
- Primary route: `app/[locale]/(page)/student/documents/page.tsx` orchestrates data fetch, filters, and action triggers.
- Interactive components: `DocumentCard.tsx`, `DocumentModal.tsx`, `DocumentsEmpty.tsx`, `DocumentsError.tsx`, `DocumentsFilters.tsx`, `DocumentsHeader.tsx`, `DocumentsLoading.tsx`, `index.ts`.
- Modal flows: `DocumentModal.tsx`. Trigger from toolbar/row actions, submit via schema-backed forms, refetch queries on success, and close modals optimistically.
...
- lib/api: `documentsApi.ts`, `index.ts`.
- lib/constants: `index.ts`.
- lib/hooks: `index.ts`, `useDocumentTypes.ts`, `useDocuments.ts`, `useDocumentsQuery.ts`.
- lib/types: `index.ts`, `types.ts`.
- lib/utils: `documentUtils.ts`, `index.ts`.
- Shared UI primitives live in `app/components/ui`; stay consistent with Button, Dropdown, SearchInput, ResizableTable, etc.
state_management:
- Data layer uses React Query via custom hooks (`index.ts`, `useDocumentTypes.ts`, `useDocuments.ts`, `useDocumentsQuery.ts`). Keep query keys in `lib/api/queryKeys` and memoize filters.
...
form_validations:
- Forms live inside `DocumentModal.tsx`. Use `react-hook-form` + `yup`/`zod`, surface translations via `next-intl`.
- Pre-fill edit modals via `reset` and keep date/number parsing centralized.
api_integration:
- REST clients live in lib/api (`documentsApi.ts`, `index.ts`). They wrap axios instance (`@/lib/api/client`) and normalize camelCase/PascalCase payloads.
...
- Pull copy from `next-intl` namespaces (e.g., `student.documents`) and keep translation keys synchronized across components and modals.
