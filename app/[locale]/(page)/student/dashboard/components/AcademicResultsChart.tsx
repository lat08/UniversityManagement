...
const AcademicResultsChart = memo(({ semesters, semesterId }: SemesterChartData) => {
  ...
  const tDashboard = useTranslations('student.dashboard');
  ...
  const emptyStatePlugin = useMemo<Plugin<"bar">>(
    ...
    [chartColors.textSecondary, hasCourses, t],
  );
  ...
  const chartOptions = useMemo<ChartOptions<"bar">>(
      ...
      plugins: {
        ...
        tooltip: {
          ...
          external: function (context) {
            ...
            const status = isPassed ? t('status.passed') : t('status.failed');
            ...
            ">
              ${course?.subjectCode ? `<div><strong>${t('columns.courseCode')}:</strong> ${course.subjectCode}</div>` : ""}
              ${course?.subjectName ? `<div><strong>${t('columns.courseName')}:</strong> ${course.subjectName}</div>` : ""}
              ${course?.credits ? `<div><strong>${t('columns.credits')}:</strong> ${course.credits}</div>` : ""}
              <div><strong>${t('gpa')}:</strong> ${score.toFixed(1)}</div>
              <div><strong>${t('statusLabel')}:</strong> ${status}</div>
            </div>
          `;
    ...
    [chartColors.primary, chartColors.tooltipBg, chartColors.tooltipText, semester?.courses, isMobile, t],
  );
    ...
    <Card className="shadow-sm h-full flex flex-col">
      <CardHeader className="pb-4 flex-shrink-0">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <CardTitle className="text-sm lg:text-base font-semibold">{t('title')}</CardTitle>
          <div className="w-full sm:w-auto">
            <Dropdown
              ...
              placeholder={tDashboard('selectSemester')}
      ...
      <CardContent className="flex-1">
        {isPending ? (
          <div className="h-[300px] lg:h-[350px] flex items-center justify-center">
            <span className="text-[var(--text-secondary)] text-sm">{tCommon('actions.loading')}</span>
          </div>
        ) : error ? (
          <div className="h-[300px] lg:h-[350px] flex items-center justify-center">
            <span className="text-[var(--error)] text-sm">{t('loadError')}</span>
          </div>
