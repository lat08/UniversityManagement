...
const RoomCard = memo(({ room, index, onBookRoom }: { 
  ...
  const roomTypeBadge = useMemo(() => ({
    text: t(`roomTypes.${room.roomType}`) || t('roomCard.unknown'),
    color: ROOM_TYPE_COLORS[room.roomType] || 'bg-gray-100 text-gray-800'
  }), [room.roomType, t]);

  const roomStatusBadge = useMemo(() => ({
    text: t(`roomStatuses.${room.roomStatus}`) || t('roomCard.unknown'),
      ...
      <CardContent className="p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className="space-y-3">
            <div>
              <p className="text-sm text-gray-700 mb-1">{t('roomCard.capacity')}</p>
              <p className="text-sm font-bold text-gray-900">{t('roomCard.capacityValue', { count: room.capacity })}</p>
            </div>
            <div>
              <p className="text-sm text-gray-700 mb-1">{t('roomCard.location')}</p>
          ...
          <div className="flex flex-col space-y-2 w-full">
            <p className="text-sm text-gray-700 mb-2 text-center">{t('roomCard.amenities')}</p>
              ...
              <div className="flex flex-col items-center gap-2 w-full max-h-[120px] overflow-y-auto">
                ...
                {room.amenities.length > 3 && (
                  <Badge className="text-xs bg-gray-50 text-gray-600 border-0 px-2 py-1 w-full flex justify-center pointer-events-none">
                    {t('roomCard.moreAmenities', { count: room.amenities.length - 3 })}
                  </Badge>
                )}
              </div>
            ) : (
              <p className="text-xs text-gray-400 text-center">{t('roomCard.noAmenities')}</p>
            )}
          </div>
        </div>
        ...
        >
          {isDisabled ? t('roomCard.unavailable') : t('roomCard.book')}
        </Button>
      </CardContent>
    </Card>
  );
    ...
    <>
      <div className="mb-4">
        <p className="text-sm text-gray-600">
          {t('roomList.summary', { count: pagination?.totalItems ?? rooms.length })}
          {pagination && (
            <span className="ml-1">
              {t('roomList.pageInfo', { current: pagination.currentPage, total: pagination.totalPages })}
            </span>
          )}
        </p>
      </div>
      ...
      {rooms.length === 0 && !isLoading && (
        <div className="text-center py-12 text-gray-500">
          ...
          <p className="text-lg font-medium">{t('roomList.empty.title')}</p>
          <p className="text-sm mt-2">{t('roomList.empty.description')}</p>
        </div>
      )}
