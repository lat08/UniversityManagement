...
export default function BookingHistory({ bookings, isLoading, pagination, currentPage, onPageChange }: BookingHistoryProps) {
  ...
  const handleConfirmCancel = () => {
    ...
    cancelBookingMutation.mutate(selectedBooking.bookingId, {
      ...
      onError: (error: unknown) => {
        ...
        const errorMessage = apiError?.response?.data?.message || apiError?.message || t('toast.cancelError');
  ...
  const getCanCancelReason = (booking: BookingData) => {
    if (booking.bookingStatus === 'cancelled') {
      return t('history.reasons.cancelled');
    }
    if (booking.bookingStatus === 'completed') {
      return t('history.reasons.completed');
    }
    ...
    try {
      ...
      if (isNaN(bookingDateTime.getTime())) {
        return t('history.reasons.invalidDate');
      }
      
      const [startHour, startMinute] = booking.startTime.split(':').map(Number);
      
      if (isNaN(startHour) || isNaN(startMinute)) {
        return t('history.reasons.invalidTime');
      }
      
      bookingDateTime.setHours(startHour, startMinute, 0, 0);
      
      if (now >= bookingDateTime) {
        return t('history.reasons.passed');
      }
      ...
      if (booking.bookingStatus === 'confirmed') {
        ...
        if (hoursUntilBooking < 3) {
          ...
          return t('history.reasons.confirmedSoon');
        }
      }
      
      return null;
    } catch {
      return t('history.reasons.unknown');
    }
  };
      ...
      <div className="bg-[#E3F2FD] rounded-lg p-6">
              ...
              <div key={booking.bookingId} className="bg-white shadow-sm overflow-hidden p-5 rounded-md">
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <Badge className={`${BOOKING_STATUS_COLORS[booking.bookingStatus] || 'bg-gray-100 text-gray-800'} px-2.5 py-0.5 text-xs font-medium rounded`}>
                      {t(`bookingStatuses.${booking.bookingStatus}`) || t('roomCard.unknown')}
                    </Badge>
                  </div>

                  {/* Location */}
                <p className="text-sm text-gray-600 mb-3">
                  {t('history.location', { building: booking.building.buildingName })}
                </p>
                ...
                <div className="grid grid-cols-2 gap-x-8 gap-y-2.5 text-sm mb-4">
                  ...
                  <div className="flex items-baseline gap-2">
                    <span className="text-gray-600">{t('history.fields.createdDate')}</span>
                  ...
                  <div className="flex items-baseline gap-2">
                    ...
                    <span className="font-semibold text-gray-900 bg-gray-100 px-2 py-0.5 rounded text-xs">
                      ...
                      {/* Ngày sử dụng */}
                  <div className="flex items-baseline gap-2">
                    <span className="text-gray-600">{t('history.fields.bookingDate')}</span>
                      <span className="font-semibold text-gray-900">{formatDate(booking.bookingDate)}</span>
                      </div>

                  {/* Số người tham gia */}
                  <div className="flex items-baseline gap-2">
                    <span className="text-gray-600">{t('history.fields.studentCount')}</span>
                    <span className="font-semibold text-gray-900">
                      {t('roomCard.capacityValue', { count: booking.studentCount ?? 0 })}
                    </span>
                      </div>

                      {/* Thời gian sử dụng */}
                  <div className="flex items-baseline gap-2">
                    <span className="text-gray-600">{t('history.fields.timeRange')}</span>
                      <span className="font-semibold text-gray-900">{booking.startTime} - {booking.endTime}</span>
                      </div>

                      {/* Mục đích sử dụng */}
                  <div className="flex items-baseline gap-2">
                    <span className="text-gray-600">{t('history.fields.purpose')}</span>
                ...
                <div className="flex justify-end items-center gap-2 mt-3">
                  <button
                    ...
                    className={`px-6 py-2 text-sm font-medium rounded-md transition-colors cursor-pointer ${
                      ...
                      >
                        {cancelBookingMutation.isPending ? t('history.actions.cancelling') : t('history.actions.cancel')}
                  </button>
                </div>
              </div>
          );
        ...
        {bookings.length === 0 && !isLoading && (
          <div className="text-center py-12 text-gray-500 bg-white rounded-lg">
            ...
            <p className="text-lg font-medium">{t('history.empty.title')}</p>
        </div>
      )}
      </div>
