...
user_role: Student
screens_and_ui_flow:
- Primary route: `app/[locale]/(page)/student/departments/page.tsx` orchestrates data fetch, filters, and action triggers.
- Interactive components: `BookingHistory.tsx`, `BookingHistoryFilters.tsx`, `BookingModal.tsx`, `CancelBookingModal.tsx`, `RoomFilters.tsx`, `RoomList.tsx`.
- Modal flows: `BookingModal.tsx`, `CancelBookingModal.tsx`. Trigger from toolbar/row actions, submit via schema-backed forms, refetch queries on success, and close modals optimistically.
...
- lib/api: `rooms.api.ts`.
- lib/hooks: `useRoomBooking.ts`.
- lib/stores: `roomBookingStore.ts`.
- Shared UI primitives live in `app/components/ui`; stay consistent with Button, Dropdown, SearchInput, ResizableTable, etc.
state_management:
- Data layer uses React Query via custom hooks (`useRoomBooking.ts`). Keep query keys in `lib/api/queryKeys` and memoize filters.
- Persistent UI/filter state lives in Zustand stores (`roomBookingStore.ts`). Hydrate defaults before mount.
...
form_validations:
- Forms live inside `BookingModal.tsx`, `CancelBookingModal.tsx`. Use `react-hook-form` + `yup`/`zod`, surface translations via `next-intl`.
- Pre-fill edit modals via `reset` and keep date/number parsing centralized.
api_integration:
- REST clients live in lib/api (`rooms.api.ts`). They wrap axios instance (`@/lib/api/client`) and normalize camelCase/PascalCase payloads.
...
- Pull copy from `next-intl` namespaces (e.g., `student.departments`) and keep translation keys synchronized across components and modals.
