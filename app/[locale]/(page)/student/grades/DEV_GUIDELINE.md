...
user_role: Student
screens_and_ui_flow:
- Primary route: `app/[locale]/(page)/student/grades/page.tsx` orchestrates data fetch, filters, and action triggers.
- Interactive components: `GradeDetailModal.tsx`, `GradeFilters.tsx`, `GradeHeader.tsx`, `GradeSemesterTable.tsx`, `GradeStatsCards.tsx`, `grades-skeleton.tsx`.
- Modal flows: `GradeDetailModal.tsx`. Trigger from toolbar/row actions, submit via schema-backed forms, refetch queries on success, and close modals optimistically.
...
- lib/types: `types.ts`.
- lib/utils: `gradeUtils.ts`, `transformers.ts`.
- Shared UI primitives live in `app/components/ui`; stay consistent with Button, Dropdown, SearchInput, ResizableTable, etc.
state_management:
- Data layer uses React Query via custom hooks (`useGrades.ts`). Keep query keys in `lib/api/queryKeys` and memoize filters.
...
form_validations:
- Forms live inside `GradeDetailModal.tsx`. Use `react-hook-form` + `yup`/`zod`, surface translations via `next-intl`.
...
- Pull copy from `next-intl` namespaces (e.g., `student.grades`) and keep translation keys synchronized across components and modals.
