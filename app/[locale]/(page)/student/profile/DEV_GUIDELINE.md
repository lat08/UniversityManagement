...
user_role: Student
screens_and_ui_flow:
- Primary route: `app/[locale]/(page)/student/profile/page.tsx` orchestrates data fetch, filters, and action triggers.
- Interactive components: `PasswordForm.tsx`, `ProfileSkeleton.tsx`.
- Entry screens must set page titles via `usePageTitle` and wrap async sections with skeleton fallbacks.
component_structure:
- UI layer split across: 2 components under `components`. Prioritize composition from table/list containers down to action menus.
...
form_validations:
- Forms live inside `PasswordForm.tsx`. Use `react-hook-form` + `yup`/`zod`, surface translations via `next-intl`.
...
- Pull copy from `next-intl` namespaces (e.g., `student.profile`) and keep translation keys synchronized across components and modals.
