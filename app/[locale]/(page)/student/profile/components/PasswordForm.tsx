...
const PasswordFormComponent = ({ onSubmit, disabled }: PasswordFormProps) => {
  ...
  const handleSavePassword = async (): Promise<void> => {
    if (!passwordData.oldPassword || !passwordData.newPassword || !passwordData.confirmPassword) {
      setPasswordError(t('profile.changePassword.fillAllFields'))
      return
    }
    ...
    if (passwordData.newPassword.length < 8) {
      setPasswordError(t('profile.changePassword.passwordMinLength'))
      return
    }
    ...
    try {
      ...
      setPasswordError(t('profile.changePassword.changePasswordError'))
    } finally {
  ...
  return (
    <div className="w-full max-w-xl mx-auto space-y-6">
      <div className="mb-2">
        <h2 className="text-base font-bold">{t('profile.changePassword.title')}</h2>
      </div>
      {passwordError && (
        ...
        >
          {isDisabled ? (
            <>
              ...
              {t('profile.changePassword.processing')}
            </>
          ) : (
            t('profile.changePassword.saveChanges')
          )}
        </Button>
      </div>
    </div>
