feature_name: Exam Schedule
user_role: Student
screens_and_ui_flow:
- Primary route: `app/[locale]/(page)/student/exam-schedule/page.tsx` orchestrates data fetch, filters, and action triggers.
- Interactive components: `ExamStatCard.tsx`, `ExamTimeline.tsx`, `NotesSection.tsx`.
- Entry screens must set page titles via `usePageTitle` and wrap async sections with skeleton fallbacks.
component_structure:
- UI layer split across: 3 components under `components`. Prioritize composition from table/list containers down to action menus.
- lib/api: `examScheduleApi.ts`.
- lib/hooks: `index.ts`, `useExamSchedule.ts`, `useNotes.ts`.
- lib/types: `types.ts`.
- lib/utils: `examUtils.ts`.
- Shared UI primitives live in `app/components/ui`; stay consistent with Button, Dropdown, SearchInput, ResizableTable, etc.
state_management:
- Data layer uses React Query via custom hooks (`index.ts`, `useExamSchedule.ts`, `useNotes.ts`). Keep query keys in `lib/api/queryKeys` and memoize filters.
...
api_integration:
- REST clients live in lib/api (`examScheduleApi.ts`). They wrap axios instance (`@/lib/api/client`) and normalize camelCase/PascalCase payloads.
...
- Pull copy from `next-intl` namespaces (e.g., `student.exam-schedule`) and keep translation keys synchronized across components and modals.
