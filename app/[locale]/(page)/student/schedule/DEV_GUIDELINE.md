...
user_role: Student
screens_and_ui_flow:
- Primary route: `app/[locale]/(page)/student/schedule/page.tsx` orchestrates data fetch, filters, and action triggers.
...
- lib/hooks: `useScheduleConflicts.ts`, `useScheduleData.old.ts`, `useScheduleData.ts`, `useSemesterSchedule.old.ts`, `useSemesterSchedule.ts`.
...
state_management:
- Data layer uses React Query via custom hooks (`useScheduleConflicts.ts`, `useScheduleData.old.ts`, `useScheduleData.ts`, `useSemesterSchedule.old.ts`, `useSemesterSchedule.ts`). Keep query keys in `lib/api/queryKeys` and memoize filters.
...
- Pull copy from `next-intl` namespaces (e.g., `student.schedule`) and keep translation keys synchronized across components and modals.
